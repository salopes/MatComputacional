function raiz = pviTaylor1234Ordens_Livro2 (funcao, xi, yi, distH, npontos, a, b)
  ...
  % ------
  dif1  = diff(funcao, x);
  fde1  = strrep (char(dif1), "**", "^");
   fPv1 = strcat(fde1, " + ( ", funcao, ")");
     d1 = inline (fPv1);

  dif2  = diff(fPv1, x);
  fde2  = strrep (char(dif2), "**", "^");
   fPv2 = strcat(fde2, " + ( ",  funcao, ")");
     d2 = inline (fPv2);

  dif3  = diff(fPv2, x);
  fde3  = strrep (char(dif3), "**", "^");
   fPv3 = strcat(fde3, " + ( ",  funcao, ")");
     d3 = inline (fPv3);
  ...
  for iCont=1: npontos
    ...
    yEuler1 = yEuler1 + distH*feval(f,xEuler,yEuler1,0);
    yEuler2 = yEuler2 + distH*(distH*feval(f,xEuler,yEuler2,0) + ((distH^1)/2)*d1(xEuler, yEuler2));
    yEuler3 = yEuler3 + distH*(distH*feval(f,xEuler,yEuler2,0) + ((distH^1)/2)*d1(xEuler, yEuler3) + ((distH^2)/6)*d2(xEuler, yEuler3));
    yEuler4 = yEuler4 + distH*(distH*feval(f,xEuler,yEuler2,0) + ((distH^1)/2)*d1(xEuler, yEuler4) + ((distH^2)/6)*d2(xEuler, yEuler4) + ((distH^3)/24)*d3(xEuler, yEuler4));
